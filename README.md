...
Within the OnDemand application, click on the Interactive Apps dropdown and select Jupyter Notebook in the Servers section.  Then fill out the Jupyter Notebook slurm submission form as follows:  select `hwgui` or `gpu` from the dropdown menu for the *Cluster partition* field.  In the *Generic Resource Request* field, enter `gpu:geforce:1`.  Note: specify `18` GB of memory in the Maximum Memory field for all worksheets used in this mini-course.
